...
// 在所有测试执行之前，初始化一下内容
func (s *ArticleMongoDBTestSuite) SetupSuite() {
...
// 每个测试用例执行完都会执行这个方法
func (s *ArticleMongoDBTestSuite) TearDownTest() {
...
func (s *ArticleMongoDBTestSuite) TestEdit() {
	t := s.T()
	testCases := []struct {
		name   string
...
func (s *ArticleMongoDBTestSuite) TestArticle_Publish() {
	t := s.T()
