...
import (
	...
	startup2 "geektime/webook/interactive/integration/startup"
...
func (s *InteractiveTestSuite) TestIncrReadCnt() {
	...
	// 不需要每个测试都创建
	svc := startup2.InitInteractiveGRPCService()
...
func (s *InteractiveTestSuite) TestLike() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			_, err := svc.Like(context.Background(), &intrv1.LikeRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
				Uid:   tc.uid,
			})
...
func (s *InteractiveTestSuite) TestDislike() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			_, err := svc.CancelLike(context.Background(), &intrv1.CancelLikeRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
				Uid:   tc.uid,
			})
	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				//查询缓存interactive:test:1 collect_cnt
				//因为key不存在，而没有执行
